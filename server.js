...
function getActorRunUrl() {
    const [user, actor] = APIFY_ACTOR_NAME.split('/');
    return `https://api.apify.com/v2/actors/${user}~${actor}/runs?token=${APIFY_TOKEN}&waitForFinish=true`;
}
...
app.post('/scrape', async (req, res) => {
    ...
    try {
        const response = await axios.post(getActorRunUrl(), { startUrls: [url] }, {
